...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "張三不喜歡買衛生紙":
        if CHATBOT:
            ...
            resultDICT["FT"].append("什麼" + inputSTR.replace(args[0], ""))

    if utterance == "張三應該會買衛生紙":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
...
if __name__ == "__main__":
    ...
    resultDICT = getResult("張三買了衛生紙", "張三買了衛生紙", [], {}, {})
    pprint(resultDICT)